...
export function generateCSS(props: GenerateCSSProps) {
  const { isChecked, size, gradientLeftColor } = props;

  function backgroundAnimation() {
    let transform = "";
    if (isChecked) transform = `translateX(-${size / 2}px)`;
    if (!isChecked) transform = `translateX(-${size * 2 - size / 2}px)`;
    return { transform };
  }
  ...
  const background_container: CSSProperties = {
  ...
  const circle_container: CSSProperties = {
    background: "white",
    position: "absolute",
    top: "0",
    bottom: "0",
